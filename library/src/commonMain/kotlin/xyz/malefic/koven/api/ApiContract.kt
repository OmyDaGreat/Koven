 ...
 * @param requiredCookies The set of required cookies for the API endpoint.
 * @param queryParams The set of allowed query parameter keys for the API endpoint.
...
abstract class ApiContract<Req, Res, ReqH : HeaderProvider, ResH : HeaderProvider, PathP : PathProvider, QueryP : QueryProvider>(
    ...
    val requiredCookies: List<CookieField<*>> = emptyList(),
 ...
 */
@Suppress("UNCHECKED_CAST")
inline fun <reified Req, reified Res> apiContract(path: String): ApiContractBuilder<Req, Res, Empty, Empty, Empty, Empty> {
    val reqFormat =
